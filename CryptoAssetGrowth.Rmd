...
3. To estimate the size of single user interaction with network let's multiply the mean value of a single transfer in USD by the expected number of transfers per user (total number of transfers / estimated number of users)
...
download <- getURL("https://community-api.coinmetrics.io/v2/assets/btc/metricdata.csv?metrics=AdrActCnt,TxTfrValMeanUSD,TxTfrCnt&start=2010-01-01")
...
data_by_week = group_by(data, year_week) %>% summarise(
  ...
  mean_transfer_val_USD = mean(TxTfrValMeanUSD),
  mean_transfer_count = mean(TxTfrCnt)) %>% mutate(
    log_mean_usage = log(mean_transfer_val_USD * mean_transfer_count / q3_adress_count),
